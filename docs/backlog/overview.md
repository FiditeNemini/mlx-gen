...
| 0113 | [Realtime video previews with tiny decoders](proposed/0113_video_family_tiny_previews.md) | Previews, Wan/Qwen/FIBO, TAEHV port, application UX | Promote when applications need live video progress: a video clip decode costs ~4.4 denoising steps against ~0.9 for an image, so tiny decoders are what make continuous video preview feasible; mapping is verified by VAE lineage and the work is porting the causal TAEHV graph plus per-family validation. |
...
| 0093 | [Sequential weight prefetch at model load](completed/0093_weight_prefetch_at_load.md) | Cold-start latency, weight loading, embedding hosts | 2026-07-25 (released in 0.25.0) | HF-repo weight files are sequentially prefetched into the OS page cache before first use (measured mechanism: page-cold random access ran ~12x faster after a 0.9 s sequential prefetch, bytes identical); prepared MLX-Gen packages deliberately excluded (measured cold-load regression); `mincore` residency skip, half-RAM guard, `MFLUX_NO_WEIGHT_PREFETCH=1` opt-out. |
| 0094 | [Default MLX buffer-cache limit for Python API and CLI](completed/0094_default_mlx_cache_limit.md) | Memory defaults, embedding hosts, system health | 2026-07-25 (released in 0.25.0) | When no limit is set, model load applies `total RAM / 8` clamped to `[1, 8] GiB` once per process with one stderr line (previously a bare Python-API session grew the cache unbounded to a measured 32.9 GB); host-managed `mx.set_cache_limit` is detected and preserved; `--mlx-cache-limit-gb` / `MFLUX_MLX_CACHE_LIMIT_GB` / `-1` unlimited opt-out win over the default. |
| 0095 | [flux2 prompt cache and compiled-predict reuse](completed/0095_flux2_prompt_cache_and_compile_reuse.md) | Fixed per-call costs, flux2, resident hosts | 2026-07-25 (released in 0.25.0) | The write-only flux2 `prompt_cache` is now read (bounded 8-entry LRU; identical prompts on a resident instance skip the ~1 s Qwen3 re-encode across txt2img/edit/inpaint/outpaint), and the per-call `mx.compile` of the denoise predict is cached per instance with invalidation on transformer replacement, LoRA application/bake, training preview, and low-RAM release. |
| 0097 | [Wan A14B i2v `last_image` bracket conditioning](completed/0097_wan_last_image_bracket_conditioning.md) | Wan video, chained-scene identity, embedding hosts | 2026-07-25 (released in 0.25.0, EXPERIMENTAL) | `--last-image` / `last_image_path` ports the diffusers first+last bracket exactly, fails loudly off the A14B 36-channel i2v layout, and ships the additive `supports_last_image` capability field (schema_version 4 -> 5); one measured Lightning 4-step probe pair (end-frame MAE 4.6/255 vs first-frame-only 56.1, no mid-clip exposure jumps) — broader recipes remain unverified. |
| 0098 | [Warn on silent UMT5 prompt truncation](completed/0098_wan_prompt_truncation_warning.md) | Wan video, prompt contract truthfulness | 2026-07-25 (released in 0.25.0) | Wan encoding measures real token counts, prints one stderr warning per truncated prompt naming the counts, and records `prompt_tokens`/`prompt_truncated` (plus the negative-prompt pair when CFG encodes it) in metadata; the 0101 smoke exercised the rewritten path end to end with bitwise-identical output. |
| 0099 | [Wan step-distill scheduler mode (`denoising_step_list`)](completed/0099_wan_step_distill_scheduler_mode.md) | Wan video, sampling parity, drift-vs-cost | 2026-07-25 (released in 0.25.0) | `--denoising-step-list` runs exact distill timestep grids on both unipc and euler solvers, mutually exclusive with `--steps`/`--flow-shift`, rejected on video-to-video and VACE, recorded in metadata and replayed by `--config-from-metadata`; the grid-honest Lightning-vs-base chain A/B stays open in the item (loader question stays in 0041). |
