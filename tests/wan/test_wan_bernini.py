...
def _guidance_model(monkeypatch, predictions):
    ...
    def fake_predict(self, *, role, target, condition_segments, source_ids, text_embeds, **kwargs):
        ...
        return mx.full(target.shape, predictions[role], dtype=mx.float32)
...
def test_bernini_r2v_uses_empty_reference_reference_text_chained_apg(monkeypatch):
    ...
    expected = 1.0 + 2.5 * (2.0 - 1.0) + 3.0 * (4.0 - 2.0)
...
def test_bernini_v2v_apg_keeps_video_in_both_text_branches(monkeypatch):
    predictions = {"v2v-empty-text": 2.0, "v2v-video-text": 5.0}
    model, calls = _guidance_model(monkeypatch, predictions)
    target = mx.zeros((1, 2, 2, 2, 2), dtype=mx.float32)
    video = mx.zeros((1, 2, 3, 4, 4))
    positive = mx.ones((1, 4, 3))
    ...
    assert np.asarray(actual) == pytest.approx(2.0 + 3.0 * (5.0 - 2.0))
...
def test_bernini_reference_images_are_preprocessed_and_vae_encoded_independently(monkeypatch, tmp_path):
    ...
    video, references, metadata = model._prepare_condition_latents(
        video_path=None,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"image_path": "first.png"}, "reference_image_paths"),
...
def test_bernini_requires_a_real_condition_source_before_model_access():
    model = BerniniRenderer.__new__(BerniniRenderer)
    with pytest.raises(ValueError, match="source video and/or"):
...
def test_bernini_metadata_records_role_truth_and_no_warm_start(tmp_path):
    ...
    assert metadata["text_encoder_precision_policy_id"] == "bernini-umt5-wo-bf16-v1"
    assert metadata["transformer_precision_policy_id"] == "bernini-transformer-final-modulation-fp32-v1"
    ...
    assert metadata["apg_accumulator_precision"] == "stable-float32"
...
def test_bernini_video_generation_starts_from_noise_not_scheduler_warm_start(monkeypatch, tmp_path):
    ...
    def fake_encode_prompt(self, **kwargs):
        ...
        return mx.ones((1, 2, 3)), mx.zeros((1, 2, 3))
    ...
    monkeypatch.setattr(
        BerniniRenderer,
        "_v2v_noise_prediction",
        lambda self, target, **kwargs: mx.zeros_like(target),
    )
    monkeypatch.setattr(
        VideoUtil,
