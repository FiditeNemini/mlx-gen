...
- [LoRA](lora.md): experimental route capability fields, explicit adapter download, strict scale matching, model-card base-model compatibility, source/no-LoRA/with-LoRA validation, exact q8 proof rows for original Qwen Image Edit, Qwen 2509/2511, Qwen 2512, Z-Image, FLUX.2 Klein, ERNIE Image Turbo, and all current Wan q8 video routes, plus the remaining base-Qwen experimental limits and the current Bonsai fail-closed boundary.
