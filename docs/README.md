...
- [API and CLI](api.md): public `mlxgen` command surface, legacy-command migration boundary, generation router behavior, image-to-image modes and canvas policy, Qwen structured control with `--controlnet-image-path`, experimental generative reframe and backend-specific outpaint, SeedVR2 image/video restoration sizing and clip controls, negative prompts including the FLUX.2 exception, Qwen edit variants, Wan video size rules, model-management commands, and Python integration boundary.
- [Image edit modes](image-edit-modes.md): plain-language guide to latent img2img, edit-reference, masked edit/inpaint, Qwen structured control, multi-reference, generative reframe, and outpaint, including what each mode is good at and what to expect from the output.
- [Qwen localized editing](qwen-localized-editing.md): plain-language explanation of Qwen masked edit today versus structured control today versus the planned control-inpaint slice, including definitions of ControlNet and “sidecar”, a visual comparison sheet, and when each route is the right tool.
- [Wan video](wan-video.md): practical Wan2.2 T2V/I2V sizing, broader A14B target size families, 5-second M5 Max comparison clips, and included MP4/frame-strip assets.
...
- [Image upscaling](upscaling.md): SeedVR2 command usage for image and video restoration, published 3B/7B q8/q4 package handles, shortest-edge and scale-factor sizing, long-video chunk controls, the validated full Eiffel 3B/7B proof videos plus sampled restore metrics, and real 5x image comparisons from a `133x113` source.
- [Image edit capabilities](edit-capabilities.md): current image-edit plus experimental generative reframe and outpaint contact sheets, exact model/package status, the Qwen Image Edit 2511 q8 masked-edit proof, the exact base Qwen q8 structured-control proof, the FLUX.2 Klein base source-model starship proof, and command logs for Qwen Image Edit, Qwen Image Edit 2509/2511, FLUX.2 Klein, Qwen structured control, and latent I2I rows. FIBO Edit is documented as unsupported through unified `mlxgen generate` in the current release.
...
- [LoRA](lora.md): experimental route capability fields, explicit adapter download, strict scale matching, model-card base-model compatibility, source/no-LoRA/with-LoRA validation, exact q8 proof rows for original Qwen Image Edit, Qwen 2509/2511, Qwen 2512, the base Qwen q8 structured-control row, Z-Image, FLUX.2 Klein, ERNIE Image Turbo, and all current Wan q8 video routes, including direct `lightx2v/Wan2.2-Lightning` download, recommended `4`-step `lightx2v/Qwen-Image-2512-Lightning`, `lightx2v/Qwen-Image-Edit-2511-Lightning`, and `lightx2v/Qwen-Image-Lightning` examples, the Qwen Image Edit 2511 q8 masked-edit Lightning proof, the exact Qwen control Lightning proof with the InstantX union ControlNet sidecar, stable `repo:subdir/file.safetensors` and absolute local-file usage for Wan A14B, same-seed no-LoRA-versus-Lightning A/B sheets, the documented q8-vs-BF16 `720p` keyframe comparison, readable `41`-frame T2V/I2V progress matrices, the `240p`-versus-`480p` T2V sweep, and the current guidance that MLX-Gen q8 packages are the validated Lightning target rather than arbitrary external FP8 checkpoints.
...
- [FAQ](faq.md): common questions about `prepare`, downloads, package naming, image-to-image mode selection and output sizing, SeedVR2 image/video restoration sizing and audio behavior, Qwen edit variants, negative prompts, experimental generative reframe, experimental canvas outpaint, Wan resolutions, compatibility, and Wan image-to-video prompting.
