...
- [API and CLI](api.md): public `mlxgen` command surface, generation router behavior, image-to-image modes and canvas policy, Qwen structured control with `--controlnet-image-path`, experimental generative reframe and backend-specific outpaint, SeedVR2 upscale sizing, negative prompts, Qwen edit variants, Wan video size rules, model-management commands, and Python integration boundary.
