...
| 0097 | [Wan A14B i2v `last_image` bracket conditioning](proposed/0097_wan_last_image_bracket_conditioning.md) | Wan video, chained-scene identity, embedding hosts | Implemented (pending release), 2026-07-25 cycle-1 wave: diffusers-exact bracket port + `--last-image`, shipped as EXPERIMENTAL with the measured probe recorded in the item. |
| 0098 | [Warn on silent UMT5 prompt truncation](proposed/0098_wan_prompt_truncation_warning.md) | Wan video, prompt contract truthfulness | Implemented (pending release), 2026-07-25 cycle-1 wave: stderr warning + `prompt_tokens`/`prompt_truncated` metadata. |
| 0099 | [Wan step-distill scheduler mode (`denoising_step_list`)](proposed/0099_wan_step_distill_scheduler_mode.md) | Wan video, sampling parity, drift-vs-cost | Implemented (pending release), 2026-07-25 cycle-1 wave: exact grids on both solvers + `--denoising-step-list`; the grid-honest Lightning-vs-base chain A/B stays open (loader question stays in 0041). |
