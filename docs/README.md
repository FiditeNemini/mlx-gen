...
- [API and CLI](api.md): public `mlxgen` command surface, generation router behavior, image-to-image modes and canvas policy, generative reframe and validated canvas outpaint, SeedVR2 upscale sizing, negative prompts, Qwen edit variants, Wan video size rules, model-management commands, and Python integration boundary.
...
- [Image edit capabilities](edit-capabilities.md): current image-edit, generative reframe, and canvas outpaint contact sheets, exact model/package status, and command logs for Qwen Image Edit, Qwen Image Edit 2509/2511, FLUX.2 Klein, and latent I2I rows. FIBO Edit is documented as unsupported through unified `mlxgen generate` in the current release.
- [Reframe and outpaint](reframe-outpaint.md): supported models, copy/pasteable `--reframe-padding` and `--outpaint-padding` examples, current source/q8/q4 contact sheets, and the validation profile id for canvas expansion workflows.
...
- [FAQ](faq.md): common questions about `prepare`, downloads, package naming, image-to-image mode selection and output sizing, SeedVR2 upscale sizing, Qwen edit variants, negative prompts, generative reframe, canvas outpaint, Wan resolutions, compatibility, and Wan image-to-video prompting.
