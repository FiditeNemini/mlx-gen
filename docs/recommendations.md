...
| --- | --- | --- | --- | --- |
| `18 GB` | Start with `prism-ml/bonsai-image-ternary-4B-mlx-2bit`, `AbstractFramework/flux.2-klein-4b-8bit`, or `AbstractFramework/ernie-image-turbo-8bit`. | Use `ernie-image-turbo-8bit` for latent restyle and `flux.2-klein-4b-8bit` for lighter edit/reference work. Avoid Qwen edit and control routes here. | No production recommendation. Bernini-R 1.3B fits the tested low-RAM memory shapes but remains experimental despite accepted official-example proof at full profile. | `seedvr2-3b` and `seedvr2-7b` image upscaling packages fit comfortably. Treat published SeedVR2 video restore as a higher-memory workflow. |
