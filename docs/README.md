...
- [LoRA](lora.md): experimental route capability fields, explicit adapter download, strict scale matching, model-card base-model compatibility, source/no-LoRA/with-LoRA validation, exact q8 proof rows for Qwen, Z-Image, FLUX.2 Klein, and ERNIE Image Turbo, current base-Qwen/original-Qwen experimental limits, Wan video LoRA status, and the current Bonsai fail-closed boundary.
