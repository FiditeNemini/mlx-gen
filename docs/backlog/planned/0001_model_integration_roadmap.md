...
| P3 deferred | T2I, I2I/edit | `briaai/FIBO`, `briaai/Fibo-lite`, `briaai/Fibo-Edit` | Base FIBO text-to-image is available, but FIBO Edit remains unsupported through unified `mlxgen generate` and has unresolved source-route parity work in item 0027. | Medium to high | Keep FIBO available only where already proven, and do not prioritize additional FIBO Edit work ahead of outpaint/reframe, LoRA strictness, or video quality work. Publish only gated derivatives with Bria license terms if this track resumes. |
...
- [ ] Deferred: revisit FIBO family validation only if item 0027 is re-prioritized or a concrete
      gated FIBO publishing need appears.
